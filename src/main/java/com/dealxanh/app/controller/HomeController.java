...
public class HomeController {
    ...
    /**
     * Model attribute — supplements GlobalModelAdvice with OTP-related fields.
     * GlobalModelAdvice already provides: notifCount, currentUserFullName,
     * currentUserInitials, currentUserAvatar, currentUserEmail.
     * This method only adds emailVerified and userEmail (OTP popup).
     */
    @ModelAttribute
    ...
    public String home(Model model, jakarta.servlet.http.HttpServletRequest request) {
        ...
        // ===== BATCH QUERY: single query replaces N+1 deal→dealProduct + dealProducts.forEach →
        // buildSaleProductCard =================================================================
        List<DealProduct> homeDealProducts = dealProductRepository.findHomeActiveDealProducts(now);
        for (DealProduct dp : homeDealProducts) {
            Map<String, Object> card = buildSaleProductCard(dp, now);
            if (card == null) continue;

            String cardDealType = (String) card.get("dealType");
            if ("FLASH_SALE".equals(cardDealType)) {
                allProducts.add(card);
                flashSaleProducts.add(card);
            } else if ("COMBO".equals(cardDealType)) {
                // COMBO deals: chỉ hiện trong section Combo, không lẫn vào Deal nổi bật
                comboProducts.add(card);
            } else {
                // VOUCHER, SEASONAL, ... → Deal nổi bật
                allProducts.add(card);
            }
        }

        // NOTE: Standalone COMBO products (productType = "COMBO") no longer auto-appear on home.
        // COMBO products must be assigned to a COMBO-type deal to show in comboProducts section.
        // Deal-based COMBO products are already added via findHomeActiveDealProducts above.

        // NOTE: Location-based sorting removed — ipapi.co external HTTP call was adding 3s
        // per request. Template does not use userLocation/distanceKm. Products remain sorted
        // by deal.priority DESC, dp.priority ASC from the batch query above.
        ...
        // Categories for category grid + sidebar
        List<Category> allCategories = categoryRepository.findAll();
        model.addAttribute("categories", allCategories);

        // Build sidebar category cards with product counts & icon mapping
        List<Map<String, Object>> sidebarCategories = new ArrayList<>();
        for (Category cat : allCategories) {
            List<Product> catProducts = productRepository.findByCategoryAndDeletedFalse(cat);
            long availableCount = catProducts.stream()
                    .filter(Product::isAvailable)
                    .count();
            String iconType = getCategoryIconType(cat.getName());
            Map<String, Object> card = new HashMap<>();
            card.put("id", cat.getCategoryId());
            card.put("name", cat.getName());
            card.put("count", availableCount);
            card.put("iconUrl", cat.getIconUrl());               // ảnh category từ DB
            card.put("iconSvg", getCategoryIconSvg(iconType));  // fallback SVG nếu chưa có ảnh
            sidebarCategories.add(card);
        }
        model.addAttribute("sidebarCategories", sidebarCategories);

        // ===== GỢI Ý HÔM NAY (Store eager-fetched by findHomeSuggestedProducts) =====
        List<Product> suggestedProducts = productService.getHomeSuggestedProducts(now);
        List<Map<String, Object>> suggestedProductCards = new ArrayList<>();
        for (Product p : suggestedProducts) {
            if (!p.isAvailable()) continue;
            Store s = p.getStore(); // eager-fetched — no lazy load
            Map<String, Object> card = new HashMap<>();
    ...
    @ResponseBody
    public java.util.Map<String, Object> buyNow(@RequestParam Long dealId, @RequestParam Long productId,
            @RequestParam String productName, @RequestParam double salePrice, @RequestParam double originalPrice,
            @RequestParam String storeName, @RequestParam(required = false) String productImage,
            @RequestParam(required = false) Long storeId,
            @RequestParam(required = false) String dealType,
        ...
        User user = getCurrentUser(principal);
        if (user == null)
            return java.util.Map.of("success", false, "message", "Vui lòng đăng nhập để mua ngay");

        // Validate product
        Product product = productRepository.findById(productId).orElse(null);
        if (product == null || !product.isAvailable())
            return java.util.Map.of("success", false, "message", "Sản phẩm không khả dụng");

        int stock = product.getStockQuantity() != null ? product.getStockQuantity() : 0;
        if (stock <= 0)
            return java.util.Map.of("success", false, "message", "Sản phẩm đã hết hàng");

        if (quantity > stock) quantity = stock;
        if (quantity <= 0) quantity = 1;

        // Look up active deal for pricing info (same logic as CartService.getCartData)
        LocalDateTime now = LocalDateTime.now();
        String discountType = "";
        double discountValue = 0;
        String applyMethod = "";
        double effectiveSalePrice = salePrice;
        ...
        if (activeDp != null) {
            ...
            double sp = activeDp.getSalePrice() != null ? activeDp.getSalePrice() : originalPrice;
            if (!"AUTO_APPLY".equals(dpDeal.getApplyMethod()) || "FIXED".equals(dpDeal.getDiscountType())) {
                sp = originalPrice;
            }
            effectiveSalePrice = sp;
            discountType = dpDeal.getDiscountType() != null ? dpDeal.getDiscountType() : "";
            discountValue = dpDeal.getDiscountValue() != null ? dpDeal.getDiscountValue() : 0;
            applyMethod = dpDeal.getApplyMethod() != null ? dpDeal.getApplyMethod() : "";
            dealId = dpDeal.getDealId();
            dealType = dpDeal.getDealType() != null ? dpDeal.getDealType() : "";
            dealName = dpDeal.getDealName() != null ? dpDeal.getDealName() : "";
        }

        // Build buyNowItem session map
        Map<String, Object> buyNowItem = new HashMap<>();
        buyNowItem.put("productId", productId);
        buyNowItem.put("productName", productName);
        buyNowItem.put("salePrice", effectiveSalePrice);
        buyNowItem.put("originalPrice", originalPrice);
        buyNowItem.put("storeName", storeName);
        buyNowItem.put("storeId", storeId != null ? storeId : 0L);
        buyNowItem.put("productImage", productImage != null ? productImage : "");
        buyNowItem.put("dealId", dealId != null ? dealId : 0);
        buyNowItem.put("dealType", dealType != null ? dealType : "");
        buyNowItem.put("dealName", dealName != null ? dealName : "");
        buyNowItem.put("discountType", discountType);
        buyNowItem.put("discountValue", discountValue);
        buyNowItem.put("applyMethod", applyMethod);
        buyNowItem.put("quantity", quantity);

        session.setAttribute("buyNowItem", buyNowItem);

        return java.util.Map.of("success", true, "message", "Đang chuyển đến trang thanh toán...");
    }
    ...
    public java.util.Map<String, Object> availableVouchers(
        ...
        for (Deal v : vouchers) {
            ...
            // Check max usage count (global limit)
            boolean usageExhausted = false;
            if (v.getMaxUsageCount() != null && v.getMaxUsageCount() > 0) {
                long usedCount = orderRepository.countByCouponCode(v.getDealCode().trim().toUpperCase());
                if (usedCount >= v.getMaxUsageCount()) {
                    usageExhausted = true;
                }
            }
            ...
            if (user != null && v.getDealCode() != null) {
                alreadyUsed = orderRepository.existsByUserUserIdAndCouponCode(
                    user.getUserId(), v.getDealCode().trim().toUpperCase());
            }

            // Check minimum order amount
            boolean meetsMinOrder = true;
            if (cartTotal != null && v.getMinOrderAmount() != null && v.getMinOrderAmount() > 0) {
                meetsMinOrder = cartTotal >= v.getMinOrderAmount();
            }
            ...
            vi.put("usageExhausted", usageExhausted);
            vi.put("alreadyUsed", alreadyUsed);
            vi.put("meetsMinOrder", meetsMinOrder);
            vi.put("canApply", !usageExhausted && !alreadyUsed && meetsMinOrder);
            list.add(vi);
        }
    ...
    public String productDetail(@PathVariable Long productId, Model model) {
        Product product = productRepository.findById(productId).orElse(null);
        // Chỉ chặn sản phẩm bị xóa, vô hiệu, hoặc chưa duyệt.
        // Không dùng isAvailable() vì nó chặn stock=0 — buyer vẫn được xem trang "Hết hàng".
        if (product == null
    ...
    public String checkout(java.security.Principal principal, Model model,
        ...
        // ===== BUY-NOW MODE: single product from session, no cart involvement =====
        @SuppressWarnings("unchecked")
        Map<String, Object> buyNowItem = (Map<String, Object>) session.getAttribute("buyNowItem");
        String src = request.getParameter("src");
        if (buyNowItem != null && "buyNow".equals(src)) {
            return buildCheckoutFromBuyNow(buyNowItem, session, request, user, model);
        }
        // Clean up stale buyNowItem if entering checkout from cart page
        if (buyNowItem != null) {
            session.removeAttribute("buyNowItem");
        }
        // ===== CART MODE: normal flow =====
        ...
        for (Long storeId : storeGroups.keySet()) {
            ...
            if (s != null) {
                ...
                si.put("phone", s.getPhone());
                si.put("openTime", s.getOpenTime());
                si.put("closeTime", s.getCloseTime());
    ...
    /**
     * Build checkout model from a single buy-now session item (no cart involvement).
     */
    private String buildCheckoutFromBuyNow(Map<String, Object> buyNowItem,
            jakarta.servlet.http.HttpSession session,
            jakarta.servlet.http.HttpServletRequest request,
            User user, Model model) {
        // Extract data from buyNowItem
        Long productId = buyNowItem.get("productId") != null ? ((Number) buyNowItem.get("productId")).longValue() : null;
        Long storeId = buyNowItem.get("storeId") != null ? ((Number) buyNowItem.get("storeId")).longValue() : null;
        int quantity = ((Number) buyNowItem.getOrDefault("quantity", 1)).intValue();
        long salePrice = ((Number) buyNowItem.getOrDefault("salePrice", 0)).longValue();
        long originalPrice = ((Number) buyNowItem.getOrDefault("originalPrice", 0)).longValue();
        String discountType = (String) buyNowItem.getOrDefault("discountType", "");
        long discountValue = ((Number) buyNowItem.getOrDefault("discountValue", 0)).longValue();
        Long dealId = buyNowItem.get("dealId") != null ? ((Number) buyNowItem.get("dealId")).longValue() : 0L;

        if (productId == null || storeId == null || storeId == 0L) {
            return "redirect:/buyer/cart";
        }

        // Build single-item cart list matching cart format
        List<Map<String, Object>> cart = new ArrayList<>();
        cart.add(buyNowItem);

        // Group by store (single store)
        Map<Long, List<Map<String, Object>>> storeGroups = new java.util.LinkedHashMap<>();
        storeGroups.computeIfAbsent(storeId, k -> new ArrayList<>()).add(buyNowItem);
        ...
        // Calculate totals (same logic as cart checkout)
        long subtotal = salePrice * quantity;
        long originalTotal = originalPrice * quantity;
        long totalDiscount = 0;
        long fixedDealDiscount = 0;
        if ("FIXED".equals(discountType) && dealId > 0) {
            fixedDealDiscount = discountValue;
        ...
        model.addAttribute("storeGroups", storeGroups);
        model.addAttribute("storeInfo", storeInfo);
        model.addAttribute("itemCount", quantity);
        model.addAttribute("subtotal", subtotal);
        model.addAttribute("totalDiscount", totalDiscount);
        model.addAttribute("voucherDiscount", voucherDiscount);
        model.addAttribute("appliedVoucher", appliedVoucher);
    ...
    /**
     * Confirm checkout from a single buy-now session item.
     * Creates Order + OrderItem without touching the cart.
     */
    @Transactional
    private String confirmBuyNowCheckout(Map<String, Object> buyNowItem,
            String paymentMethod, String scheduledPickupTime,
            jakarta.servlet.http.HttpSession session, User user) {
        // Extract data from buyNowItem
        Long productId = buyNowItem.get("productId") != null ? ((Number) buyNowItem.get("productId")).longValue() : null;
        Long storeId = buyNowItem.get("storeId") != null ? ((Number) buyNowItem.get("storeId")).longValue() : null;
        int quantity = ((Number) buyNowItem.getOrDefault("quantity", 1)).intValue();
        double salePrice = ((Number) buyNowItem.getOrDefault("salePrice", 0)).doubleValue();
        double originalPrice = ((Number) buyNowItem.getOrDefault("originalPrice", 0)).doubleValue();
        String discountType = (String) buyNowItem.getOrDefault("discountType", "");
        double discountValue = ((Number) buyNowItem.getOrDefault("discountValue", 0)).doubleValue();
        long dealId = buyNowItem.get("dealId") != null ? ((Number) buyNowItem.get("dealId")).longValue() : 0L;
        ...
        // FIFO lock to prevent double-checkout
        ...
        try {
            ...
            Store store = storeRepository.findById(storeId).orElse(null);
            if (store == null) return "redirect:/buyer/cart";
            ...
            if (scheduledPickupTime != null && !scheduledPickupTime.isEmpty()) {
                ...
                } catch (Exception e) { /* ignore invalid format */ }
            }

            // Create OrderItem
            double totalAmount = originalPrice * quantity;
            ...
            if ("FIXED".equals(discountType) && dealId > 0) {
                ...
                discountAmount = discountValue; // FIXED discount applied once
            } else {
                oi.setUnitPrice(salePrice);
                if (originalPrice > salePrice) {
                    discountAmount = (originalPrice - salePrice) * quantity; // PERCENT per-item
                }
            }
            oi.setOrder(order);
            ...
            if (voucherCode != null && voucherDiscountTotal > 0) {
            ...
            // Clean up session — NOTE: do NOT clear cart!
            session.removeAttribute("buyNowItem");
            session.removeAttribute("appliedVoucher");
            session.setAttribute("lastOrderIds", List.of(order.getOrderId()));
    ...
    public String confirmCheckout(java.security.Principal principal,
        ...
        // ===== BUY-NOW MODE: checkout from single session item, no cart =====
        @SuppressWarnings("unchecked")
        Map<String, Object> buyNowItem = (Map<String, Object>) session.getAttribute("buyNowItem");
        if (buyNowItem != null) {
            return confirmBuyNowCheckout(buyNowItem, paymentMethod, scheduledPickupTime, session, user);
        }
    ...
    public String paymentPage(java.security.Principal principal, Model model,
        ...
        // Pass buy-now totals if in buy-now mode (so payment page can show correct recap)
        @SuppressWarnings("unchecked")
        Map<String, Object> buyNowItem = (Map<String, Object>) session.getAttribute("buyNowItem");
        if (buyNowItem != null) {
            int quantity = ((Number) buyNowItem.getOrDefault("quantity", 1)).intValue();
            ...
            long subtotal = salePrice * quantity;
            long originalTotal = originalPrice * quantity;
            long fixedDealDiscount = ("FIXED".equals(discountType) && dealId > 0) ? discountValue : 0;
            long buyNowTotal = subtotal - fixedDealDiscount;

            model.addAttribute("isBuyNow", true);
            model.addAttribute("buyNowTotal", buyNowTotal);
            model.addAttribute("buyNowItemCount", quantity);
            model.addAttribute("buyNowOriginalTotal", originalTotal);
        }
    ...
    public String storeProfile(@PathVariable Long storeId,
        ...
        model.addAttribute("dealProductMap", dealProductMap);
        model.addAttribute("reviewMode", review != null && review == 1);
        model.addAttribute("reviewOrderId", orderId);
    ...
    /**
     * Build a product card Map from a DealProduct — reused by home() and the sale-products page.
     * Returns null if Product or Deal is missing (caller should skip).
     */
    private Map<String, Object> buildSaleProductCard(DealProduct dp, LocalDateTime now) {
        Product prod = dp.getProduct();
        Deal deal = dp.getDeal();
        if (prod == null || deal == null) return null;
        ...
        // Null-safe price snapshot from DealProduct
        double origPrice = dp.getOriginalPrice() != null ? dp.getOriginalPrice() : 0.0;
        double salePrice = dp.getSalePrice() != null ? dp.getSalePrice() : origPrice;
        ...
        card.put("originalPrice", Math.round(origPrice));
        card.put("salePrice", Math.round(salePrice));
        card.put("maxQuantity", dp.getMaxQuantity());
        card.put("endTime", endTimeStr);
        card.put("daysLeft", daysLeft);

        // Luôn tính % từ giá thực tế trên DealProduct (chính xác hơn dùng deal.discountValue)
        double discountPercent = 0;
        if (origPrice > 0 && salePrice < origPrice) {
            discountPercent = ((origPrice - salePrice) / origPrice) * 100;
        }
    ...
    /** Full page: Tất cả sản phẩm đang sale */
    ...
    public String saleProducts(
        ...
        int cartCount = getCartItemCount(request);
        model.addAttribute("activeNav", "sale");
        model.addAttribute("cartItemCount", cartCount);

        LocalDateTime now = LocalDateTime.now();
        var pageable = PageRequest.of(page, 24);
        Page<DealProduct> dpPage = dealProductRepository.findActiveDealProducts(now, keyword, pageable);

        List<Map<String, Object>> products = new ArrayList<>();
        for (DealProduct dp : dpPage.getContent()) {
            products.add(buildSaleProductCard(dp, now));
        }

        model.addAttribute("products", products);
        model.addAttribute("keyword", keyword != null ? keyword : "");
        model.addAttribute("currentPage", page);
        model.addAttribute("totalPages", dpPage.getTotalPages());
        model.addAttribute("hasPrev", page > 0);
        model.addAttribute("hasNext", page < dpPage.getTotalPages() - 1);
        model.addAttribute("totalItems", dpPage.getTotalElements());

        return "buyer/sale-products";
    }

    /** AJAX fragment: returns only the product grid + pagination HTML */
    @GetMapping("/sale-products/fragment")
    public String saleProductsFragment(
            @RequestParam(required = false) String keyword,
            @RequestParam(defaultValue = "0") int page,
            Model model) {
        ...
        return "buyer/sale-products :: productGrid";
    }

    /**
     * Map category name to icon type for the sidebar SVG icon.
     * Matches common Vietnamese food category name patterns.
     */
    private String getCategoryIconType(String name) {
        if (name == null) return "default";
        String n = name.toLowerCase();
        if (n.contains("ăn liền") || (n.contains("đóng gói") && n.contains("ăn"))) return "instant-food";
        if (n.contains("bánh") || n.contains("kẹo") || n.contains("ngọt")) return "sweets";
        if (n.contains("uống") || n.contains("nước") || n.contains("trà") || n.contains("cà phê")) return "drinks";
        if (n.contains("thực phẩm") || n.contains("đông lạnh") || n.contains("tươi sống")) return "packaged-food";
        if (n.contains("vặt") || n.contains("snack") || n.contains("bim bim")) return "snacks";
        if (n.contains("gia vị") || n.contains("nguyên liệu") || n.contains("nấu")) return "spices";
        if (n.contains("cơm") || n.contains("mì") || n.contains("phở") || n.contains("bún")) return "meals";
        if (n.contains("sữa") || n.contains("yaourt")) return "dairy";
        if (n.contains("trái cây") || n.contains("hoa quả") || n.contains("rau")) return "produce";
        return "default";
    }

    /**
     * Return inline SVG markup string for a given icon type.
     * All SVGs are 18x18, stroke-based, with class="dx-cat-icon-svg".
     */
    private String getCategoryIconSvg(String iconType) {
        switch (iconType) {
            case "instant-food":
                return "<svg width=\"18\" height=\"18\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"1.7\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><rect x=\"3\" y=\"4\" width=\"18\" height=\"16\" rx=\"3\"/><line x1=\"3\" y1=\"9\" x2=\"21\" y2=\"9\"/><path d=\"M8 2v3\"/><path d=\"M16 2v3\"/></svg>";
            case "sweets":
                return "<svg width=\"18\" height=\"18\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"1.7\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><circle cx=\"12\" cy=\"12\" r=\"9\"/><path d=\"M9 9h.01\"/><path d=\"M15 9h.01\"/><path d=\"M8 14s1.5 3 4 3 4-3 4-3\"/></svg>";
            case "drinks":
                return "<svg width=\"18\" height=\"18\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"1.7\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><path d=\"M8 2h8\"/><path d=\"M9 2v4.5c0 2-1 5-1 8 0 3.5 1.8 6.5 4 7.5 2.2-1 4-4 4-7.5 0-3-1-6-1-8V2\"/><rect x=\"7\" y=\"13\" width=\"10\" height=\"3\" rx=\"1\"/></svg>";
            case "packaged-food":
                return "<svg width=\"18\" height=\"18\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"1.7\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><path d=\"M6 2L3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z\"/><line x1=\"3\" y1=\"6\" x2=\"21\" y2=\"6\"/><path d=\"M10 10.5v7\"/><path d=\"M14 10.5v7\"/></svg>";
            case "snacks":
                return "<svg width=\"18\" height=\"18\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"1.7\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><path d=\"M5 3l14 7-6 3 3 8-4-2-5 2 3-6-5-3z\"/><circle cx=\"7\" cy=\"6\" r=\"1.5\"/><circle cx=\"16\" cy=\"15\" r=\"1\"/></svg>";
            case "spices":
                return "<svg width=\"18\" height=\"18\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"1.7\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><rect x=\"7\" y=\"7\" width=\"10\" height=\"14\" rx=\"2\"/><path d=\"M7 7c0-2 2-5 5-5s5 3 5 5\"/><line x1=\"7\" y1=\"12\" x2=\"17\" y2=\"12\"/><circle cx=\"10\" cy=\"16\" r=\"1\" fill=\"currentColor\"/><circle cx=\"14\" cy=\"16\" r=\"1\" fill=\"currentColor\"/></svg>";
            case "meals":
            ...
            case "dairy":
                return "<svg width=\"18\" height=\"18\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"1.7\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><path d=\"M8 2h8l2 10H6L8 2z\"/><path d=\"M6 12h12v6a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2v-6z\"/></svg>";
            case "produce":
                return "<svg width=\"18\" height=\"18\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"1.7\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><path d=\"M12 2a7 7 0 0 1 7 7c0 4.4-7 13-7 13S5 13.4 5 9a7 7 0 0 1 7-7z\"/><circle cx=\"12\" cy=\"9\" r=\"2.5\"/></svg>";
            default:
                return "<svg width=\"18\" height=\"18\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"1.7\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><rect x=\"3\" y=\"3\" width=\"7\" height=\"7\" rx=\"1\"/><rect x=\"14\" y=\"3\" width=\"7\" height=\"7\" rx=\"1\"/><rect x=\"3\" y=\"14\" width=\"7\" height=\"7\" rx=\"1\"/><rect x=\"14\" y=\"14\" width=\"7\" height=\"7\" rx=\"1\"/></svg>";
        }
    }
