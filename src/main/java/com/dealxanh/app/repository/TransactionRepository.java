...
public interface TransactionRepository extends JpaRepository<Transaction, Long> {
    ...
    // GMV = sum of all completed order amounts (case-insensitive type)
    @Query("SELECT COALESCE(SUM(t.amount), 0) FROM Transaction t WHERE UPPER(t.type) = 'SALE' AND t.status = 'COMPLETED' AND t.createdAt BETWEEN :start AND :end")
    ...
    @Query(value = "SELECT COALESCE(SUM(t.amount), 0) FROM transactions t WHERE UPPER(t.type) = 'SALE' AND t.status = 'COMPLETED' AND DATE(t.created_at) = DATE(:date)", nativeQuery = true)
    ...
    @Query(value = "SELECT t FROM Transaction t JOIN FETCH t.store WHERE t.type = 'PAYOUT' AND t.status = 'PENDING' ORDER BY t.createdAt DESC",
           countQuery = "SELECT COUNT(t) FROM Transaction t WHERE t.type = 'PAYOUT' AND t.status = 'PENDING'")
    Page<Transaction> findPendingPayoutsWithStore(Pageable pageable);

    // Paginated completed payouts
    @Query(value = "SELECT t FROM Transaction t JOIN FETCH t.store WHERE t.type = 'PAYOUT' AND t.status = 'COMPLETED' ORDER BY t.processedAt DESC, t.createdAt DESC",
           countQuery = "SELECT COUNT(t) FROM Transaction t WHERE t.type = 'PAYOUT' AND t.status = 'COMPLETED'")
    Page<Transaction> findCompletedPayoutsWithStore(Pageable pageable);
    ...
    @Query("SELECT COALESCE(SUM(t.amount), 0) FROM Transaction t WHERE t.store.storeId = :storeId AND UPPER(t.type) = 'SALE' AND t.status = 'COMPLETED'")
